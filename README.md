...
## Python Setup / Security

このリポジトリの `Python/` は、`.env` を使って秘密情報を分離して運用します。  
実値は絶対にコミットせず、共有時は `.env.example` のみを使ってください。

### セットアップ

```powershell
cd C:\Users\ron06\OneDrive\Documents\Program\GitHub\Shirafuka-Practice\Python
python -m venv .venv
...
- 新しい秘密情報ファイルを追加したら、親 `.gitignore` に除外ルールを追加する
- `.env.example` はキー名だけを管理し、値は空にする
...
### gdrive_backup_gui.py

`Add 2026/03/23`  
Google Driveバックアップ用のGUIツール。  
バックアップ元/先のフォルダー選択、`Dry Run`、安全ミラー削除（`--delete`）をGUIで実行できます。

**起動例:**

```powershell
python Python\gdrive_backup_gui.py
```
...
`Add 2026/03/02`  
音源のBPMを自動検出し、ビートに同期した写真スライドショー動画を生成するPython。  
librosaでBPMを解析し、音源の激しさに応じて写真の切り替え速度を自動調整します。

**主な機能:**

- 🎵 音源ファイルからBPMを自動検出（librosa使用）
- 🎬 ビートに同期した写真切り替え
  - **ランダムモード**: 0.5〜4拍のランダム間隔
  - **自動調整モード**: 音源の激しさに応じて切り替え速度を変更（非常に激しい箇所は0.5拍、穏やかな箇所は4拍）
- 📱 横動画（1920x1080）または縦動画（1080x1920）を選択可能
- 🎚️ 音源の使用範囲を指定可能（開始時間・終了時間）
- 🖼️ アスペクト比維持（黒帯付きフィット）
- 🔄 EXIF自動回転対応（スマートフォンで撮影した写真も正しい向きで表示）
- 📝 タイトルと日付を動画全体に常時表示
- 🇯🇵 日本語フォント対応
- 🎨 4拍分のオープニング黒背景タイトル画面
- 🌐 多形式対応（JPG, PNG, HEIC, WebP等）
...
7. 終了日付（YYYY/MM/DD）
8. フォントファイルパス（オプション）
9. 切り替えモード（1: ランダム, 2: 自動調整）

**実行結果:**

- オープニング: 4拍分の黒背景にタイトル+日付
- スライドショー: 写真の上にタイトル+日付が常時表示
- 出力例: `slideshow_横_1920x1080_128bpm_auto_2026-03-02.mp4`
...
**必要なライブラリ:** librosa, opencv-python, numpy, pillow, pillow-heif
...
- ⚡ **高速処理**: ffmpegを直接使用することで、moviepyより高速に動画を生成
- 🎬 **高品質**: libx264コーデックで高品質な動画出力
