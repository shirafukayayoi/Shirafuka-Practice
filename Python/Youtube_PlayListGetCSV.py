...
def main():
    ...
    spreadsheet_id = google_drive.create_spreadsheet(
        playlist_name
    )  # スプレッドシートを作成
...
class YoutubePlayListGet:
    ...
    def check_playlist_name(self):
        ...
        request = self.youtube_service.playlists().list(
            part="snippet", id=self.playlist_id
        )
...
class GoogleDriveAuth:
    ...
    def create_spreadsheet(self, playlist_name):
        ...
        # 新しいスプレッドシートを作成
        spreadsheet = {"properties": {"title": f"{now}_{playlist_name}"}}
    ...
    def write_csv(self, spreadsheet_id, data):
        ...
        print(
            f"{result.get('updates').get('updatedCells')} cells updated."
        )  # 更新されたセルの数を表示
